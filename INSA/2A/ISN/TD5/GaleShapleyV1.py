...
def get_les_stages_non_proposes(etu, proposes, les_stages):
    ...
    for (
        stage
    ) in les_stages:  # o(n*m), n=len(les_stages), m=len(propositions_deja_faites)
...
def get_stage_prefere_parmi(stages_possibles, prefs):
    ...
    """
    i = 0
    while i < len(prefs) and not (
        prefs[i] in stages_possibles
    ):  # o(n*m), n=len(prefs) et m=len(stage_possibles)
        i += 1
