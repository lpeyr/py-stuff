...
def get_les_stages_non_proposes(etu, proposes, les_stages) -> set:
    ...
    res = set(les_stages) - set(propositions_deja_faites)
    return res


def get_next_etudiant_disponible_et_appariable(
    les_etudiants, paires, proposes, les_stages
):
    ...
    i = 0
    while etudiant_choisi == None and i < len(
        les_etudiants
    ):  # o(n*m), n=len(les_etudiants) et m=len(list(paires.keys()))
        etu = les_etudiants[i]
        if (
            etu not in paires.keys()
            and len(les_stages) - len(proposes.get(etu, [])) >= 1
        ):
            etudiant_choisi = etu
        i += 1
...
def get_stage_prefere_parmi(stages_possibles: set, prefs):
...
def gale_shapley(etudiants, stages, prefs_etudiants, prefs_stages):
    ...
    ###### Boucle de travail
    etu = get_next_etudiant_disponible_et_appariable(
        etudiants, paires, proposes, stages
    )
    while (
        etu != None
    ):  # tant qu’il existe un étudiant "etu" non apparié et appariable (au moins 1 stage non encore essayé)
        stages_possibles = get_les_stages_non_proposes(etu, proposes, stages)
        stage_choisi = get_stage_prefere_parmi(
            stages_possibles, prefs_etudiants[etu]
        )  # Récupérer le stage préféré de l’étudiant etu
        ...
        if not stage_choisi in paires.values():  # si le stage choisi n’est pas apparié
            ...
            paires_inv[stage_choisi] = etu

        else:  # sinon, c’est qu’il existe un old_etu déjà associé à stage
            old_etu = paires_inv[stage_choisi]

            if (
                prefs_stages[stage_choisi][old_etu] < prefs_stages[stage_choisi][etu]
            ):  # si stage_choisi préfère etu plutôt que old_etu
                paires.pop(old_etu)  # old_etu devient non apparié
                paires[etu] = (
                    stage_choisi  # etu et stage_choisi sont maintenant appariés
                )
                paires_inv[stage_choisi] = etu
...
if __name__ == "__main__":
    ...
    print(
        "   Attendu  : {'Dora': 'Topographie', 'Indiana Jones': 'Archéologie', 'Bob': 'Sponge factory'}"
    )
