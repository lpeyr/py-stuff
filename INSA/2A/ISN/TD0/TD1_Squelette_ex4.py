...
def indexof(l_index, id) : # O(log_2(n))
    ...
    b = 0
    u = len(l_index) -1
    while b<=u and index==-1:
        m = (b+u)//2
        if l_index[m] == id:
            index = m
        elif int(l_index[m]) > int(id):
            u = m-1
        else:
            b = m+1  
    return index
...
def make_matrice_trajets(l_index, l_trajets) : # O(n²)
    ...
    for station in l_index:
        trajets_vers_station = {v:0 for v in l_index}      # Création d'un dictionnaire pour stocker le nombre de trajet vers la station [key]
