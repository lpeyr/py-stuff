...
if __name__ == "__main__":
    ...
    tps_calcul_algo1 = (
        []
    )  # pour stocker les temps d'exécutation pour chaque graphe avec l'algo 1
    ...
    for nom in noms_graphes:
        ...
        for algo in algos:
            if algo == "algo1":
                print(
                    "exec de l'algo 1 : solution utilisant BFS avec arbre des parents"
                )
                tmps = 0
                for _ in range(nb_repet):  # répétitions des expérimentations
                    l_noeuds_fraicheur = choisir_points_fraicheur(
                        dico_graphe, nb_pts_fraich
                    )  # tirage aléatoire des pts de fraicheur
                ...
                print(
                    "exec de l'algo 2 : solution utilisant BFS avec calcul des hauteurs"
                )
                tmps = 0
                for _ in range(nb_repet):
                    ...
                    start = time.perf_counter()
                    deploy = BFS_complet_pour_tout_h_avec_calcul_distance(
                        l_noeuds_fraicheur, dico_graphe
                    )
                    end = time.perf_counter()
                    tmps += end - start
                ...
                print(
                    "exec de l'algo 3 : solution utilisant BFS avec arret des atteinte du premier neoud de fraicheur "
                )
                tmps = 0
                for _ in range(nb_repet):
                    l_noeuds_fraicheur = choisir_points_fraicheur(
                    ...
                    start = time.perf_counter()
                    deploy = BFS_pour_tout_h_avec_arret_des_prem_nf(
                        l_noeuds_fraicheur, dico_graphe
                    )
                    end = time.perf_counter()
                    tmps += end - start
                ...
                print(
                    "exec de l'algo 4 : solution utilisant BFS à partir des noeuds de fraicheur"
                )
                tmps = 0
                for _ in range(nb_repet):
                    l_noeuds_fraicheur = choisir_points_fraicheur(
            ...
            print(
                f"Temps d'exécution moyen : {tmps/nb_repet:.5f} secondes pour {len(dico_graphe.keys())} noeuds"
            )
