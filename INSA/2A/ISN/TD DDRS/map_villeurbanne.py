...
def dictionnaire_liens(file):
    ...
    for lien in liste_liens:
        ...
        dico_liens[code] = {"nom": lien["properties"]["nom"], "commune": lien["properties"]["nomcommune"], "typecirculation": lien["properties"]["typecirculation"]}
...
def dictionnaire_noeuds(file):
    ...
    for noeud in liste_noeuds:
        ...
        if inter is not None: #On droppe les noeuds sans lien => erreur de fichier
            incidence = list(set(inter.split("|"))) #construction de l'ensemble des liens indicent au noeud, sans doublon
            dico_noeuds[id] = {"incidence":incidence, "nom":noeud["properties"]["libellecarrefour"], "geometry":noeud["geometry"]}
...
def selectionner_liens_graphe(dico_noeuds, dico_liens):
    ...
    for id, noeud in dico_noeuds.items(): #on parcours tous les sommets
        for code_lien in noeud["incidence"]:  #double boucle : passe 2 fois par lien de la métropole si tout est correct : O(|E_metropole])
            if code_lien in dico_liens: #si le lien est dans la région d'intéret
                dico_liens_graphe[code_lien] = dico_liens_graphe.get(code_lien,dico_liens[code_lien].copy()) #copy pour pas modifier le lien original et éviter les effets de bord
                dico_liens_graphe[code_lien]["extremites"] = dico_liens_graphe[code_lien].get("extremites", []) + [id] #on mémorise que id est une extrémité du lien

    #les "bons" liens sont incidents à 2 sommets. On compte ceux qui ont trop d'extremites, pas assez ou pas du tout
    nb_liens_bugs = len([code_lien for code_lien in dico_liens_graphe if len(dico_liens_graphe[code_lien]["extremites"])>2])
    nb_liens_deconnectes  = len([code_lien for code_lien in dico_liens_graphe if len(dico_liens_graphe[code_lien]["extremites"])==1])
    print(f"{nb_liens_bugs} ont trop d'extrémités, {nb_liens_deconnectes} n'en ont pas assez, {len(dico_liens)-len(dico_liens_graphe)} n'en ont pas")

    return {code_lien:dico_liens_graphe[code_lien] for code_lien in dico_liens_graphe if len(dico_liens_graphe[code_lien]["extremites"]) == 2} #generation du résultat : O(|E_quartier])
...
def construire_graphe(dico_noeuds, dico_liens_graphe):
    ...
    for code_lien,lien in dico_liens_graphe.items(): # O(|E_quartier|)
        if len(lien.get("extremites",[]))==2:
            ...
            dico_noeuds_graphe[u] = dico_noeuds_graphe.get(u, {"voisins":dict(), "incidence": [], "nom":noeud_u["nom"], "geometry":noeud_u["geometry"]})
            noeud_v = dico_noeuds[v]
            dico_noeuds_graphe[v] = dico_noeuds_graphe.get(v, {"voisins":dict(), "incidence": [], "nom":noeud_v["nom"], "geometry":noeud_v["geometry"]})
...
if __name__ == "__main__":
    ...
    liens_valides = selectionner_liens_graphe(noeuds, liens) # garde que les "bons" liens du quartier et ajoute leurs extremités
    print(len(liens_valides), "bons liens")

    noeuds_graphe = construire_graphe(noeuds, liens_valides) # construit le graphe des bons liens, y compris le voisinnage


    print("Villeurbanne :",len(noeuds_graphe), "noeuds -----", len(liens_valides), "liens")
