...
def main():
    ...
    if st.sidebar.checkbox("Affichez les données brut", False):
        st.write(data)
    all_stats = ["count", "moyenne", "écart-type", "variance", "min", "1er quartile", "médiane", "3ème quartile", "max"]
    def_stats = ["moyenne", "écart-type", "médiane"]
    list_stats_to_show = st.sidebar.multiselect("Statistiques à afficher", options=all_stats, default=def_stats,
                                                key='multi-stats')
    st.sidebar.subheader("Visualisation")
    st.sidebar.checkbox("Cacher", True)
    select = st.sidebar.selectbox('Visualisation type', ['Histogram', 'Box Plot', 'Probability Density'], key='viz')
    ...
    if select == "Histogram":
        # create a histogram
        fig = px.histogram(data["conc_pm10"][(data["conc_pm10"] < thres)],
    ...
    df_lyon = (data[data['lib_zone'].isin(list_lyon)])[["X", "Y", "lib_zone", "conc_pm10"]]
    df_grenoble = (data[data['lib_zone'].isin(list_grenoble)])[["X", "Y", "lib_zone", "conc_pm10"]]
